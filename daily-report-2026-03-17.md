...
## 📊 Capacity
- Weekly burn: 138h / 40h
- Portfolio load: 345% of weekly capacity
...
## 🟡 What's In Progress
- MASC sprint work for 2026-03-20: the calculator missing-values fix is now complete, and after `LCTX-01` the next active MASC lane is to finish `MASC-02` and then sequence the remaining plugin-owned and content-owned items.
...
## 🔴 Blockers / Risks
- Total tracked work is 138h against a 40h week, so queued work cannot all move this week.
...
- Flywheel has also flagged that strict HSTS carries a one-year browser persistence risk, so `LCTX-01` now includes a decision between enabling HSTS and using the softer server-side `Force HTTPS` option instead.
- Flywheel also clarified that CSP setup is the developer-owned part of `LCTX-01`: start with `Report-Only`, use the HTTP Headers plugin if needed, discover all required sources through console review, and only enforce once the allowlist is complete.
...
- Current MASC next steps: route Jordan-owned plugin items and content-owned fixes to the right owners, schedule the new EN feedback cleanup bundle, and get French translations for the featured block content on the French `MMPP` page
- Current MASC next steps: route Jordan-owned plugin items and content-owned fixes to the right owners, schedule the new EN feedback cleanup bundle, run `MASC-05` next week as a collaborative planning pass, and get French translations for the featured block content on the French `MMPP` page
...
- On 2026-03-20: after `LCTX-01`, move to `MASC-02` and use the remaining time to finish the cleanup bundle if possible.
